      ...
      <div className="px-5 py-5">
        {latestStrategy ? (
          <div className="space-y-4">
            ...
            {(latestStrategy.content as any)?.positioning_summary && (
              <div>
                <p className="text-xs font-medium text-gray-400 uppercase tracking-wide mb-1">Positioning</p>
                <p className="text-sm text-gray-700">{(latestStrategy.content as any).positioning_summary}</p>
              </div>
            )}
            {(latestStrategy.content as any)?.content_pillars?.length > 0 && (
              <div>
                <p className="text-xs font-medium text-gray-400 uppercase tracking-wide mb-2">Content Pillars</p>
                <div className="flex flex-wrap gap-2">
                  {(latestStrategy.content as any).content_pillars.map((p: any, i: number) => (
          ...
          <div className="text-center py-8">
            ...
            <p className="text-xs text-gray-400 max-w-xs mx-auto">
              AI strategy generation coming in the next phase. Add stories and offers first to get the best results.
            </p>
          </div>
        )}
      </div>
